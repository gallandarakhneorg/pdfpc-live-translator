...
class Voice2TextConverter(object):
	def __init__(self, cli_path : object, audio : AudioStreamer, translator : Translator, listener : Voice2TextListener):
		self._partial = cli_path.partial
		self._audio = audio
		self._translator = translator
		self._listener = listener
		self._model = Model(cli_path.langmodel)
	...
	def listen(self):
		...
		if self._recognizer.AcceptWaveform(data):
			...
			if "partial" in partial:
				if partial["partial"]:
					translated = self._translator.translate(partial["partial"])
				else:
					translated = ''
				self._listener.update_text(translated)
...
class TkListener(Voice2TextListener):
	def __init__(self, cli_args : object):
		...
		self._text_area = scrolledtext.ScrolledText(self._root,
			...
			font=(str(cli_args.font) + " " + str(cli_args.height) + " bold"))
